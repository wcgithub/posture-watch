...
Each frame, the app measures two ratios, normalized by shoulder width or
ear width so they're insensitive to how far you are from the camera:
...
This assumes a frontal (face-on) camera — a side-mounted/profile setup
isn't supported.
...
- **Show debug info** — a live panel with raw metric values, calibrated
  good/bad values, live scores, and current thresholds, each colored green
  when on the good side of its threshold and red when bad enough to alert.
  Also draws directly on the video: a dashed yellow line for the live head
  measurement (nose to shoulder-midpoint), plus green "good" and red "bad"
  reference marks reconstructed from your calibration — a horizontal tick
  where your nose should sit for good/bad head position, and a horizontal
  span showing the good/bad shoulder width — so you can see exactly how
  your current pose compares to both calibration points at a glance.
